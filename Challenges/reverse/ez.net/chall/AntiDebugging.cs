...
namespace KoitoCoco.MoeCtf
{
    ...
    public enum f : int
    {
        ...
        ProcessPriorityBoost, // qs: ULONG>essDeviceMap, // qs: PROCESS_DEVICEMAP_INFORMATION, PROCESS_DEVICEMAP_INFORMATION_EX
    ...
    public static class j
    {
        ...
        [DllImport("Kernel32.dll", SetLastError = true)]
        internal static extern bool WaitForDebugEvent([Out] out b lpDebugEvent, int dwMilliseconds);

        [DllImport("Kernel32.ddotnet get class hashll", SetLastError = true)]
        ...
        internal static extern d NtQueryInformationProcess([In] IntPtr processHandle, [In] f processInformationClass, out IntPtr processInformation, [In] int processInformationLength, out int returnLength);

        [DllImport("ntdll.dll", SetLastError = true, ExactSpelling = true)]
        internal static extern d NtClose([In] IntPtr handle);

        [DllImport("ntdll.dll", SetLastError = true, ExactSpelling = true)]
        internal static extern d NtRemoveProcessDebug(IntPtr processHandle, IntPtr debugObjectHandle);

        [DllImport("ntdll.dll", SetLastError = true, ExactSpelling = true)]
        internal static extern d NtSetInformationDebugObject([In] IntPtr debugObjectHandle, [In] g debugObjectInformationClass, [In] IntPtr debugObjectInformation, [In] int debugObjectInformationLength, [Out] out int returnLength);

        [DllImport("ntdll.dll", SetLastError = true, ExactSpelling = true)]
        internal static extern d NtQuerySystemInformation([In] h systemInformationClass, IntPtr systemInformation, [In] int systemInformationLength, [Out] out int returnLength);

        [DllImport("ntdll.dll")]
        internal static extern d NtSetInformationThread(IntPtr threadHandle, e threadInformationClass, IntPtr threadInformation, int threadInformationLength);

        [DllImport("kernel32.dll")]
        static extern IntPtr OpenThread(a dwDesiredAccess, bool bInheritHandle, uint dwThreadId);
        ...
        public static bool ii1()
        {
            var dwFlags = 0U;

            unsafe
            {
                var ntStatus = NtQueryInformationProcess(Process.GetCurrentProcess().Handle, f.ProcessDebugObjectHandle, out var hDebugObject, IntPtr.Size, out _);

                if (ntStatus != d.Success)
                {
                    return false;
                }

                ntStatus = NtSetInformationDebugObject(hDebugObject, g.DebugObjectFlags, new IntPtr(&dwFlags), Marshal.SizeOf(dwFlags), out _);
        ...
        public static bool iiii()
        {
            ...
            unsafe
            {
                ntStatus = NtQuerySystemInformation(h.SystemKernelDebuggerInformation, new IntPtr(&pSkdi),
        ...
        public static void iii1()
        {
            ...
            foreach (ProcessThread thread in currentThreads)
            {
                ...
                var pOpenThread = OpenThread(a.SetInformation, false, (uint)thread.Id);
        ...
        public static bool i1ii(IntPtr handle)
        {
            var nStatus = NtSetInformationThread(handle, e.ThreadHideFromDebugger, IntPtr.Zero, 0);

            return nStatus == d.Success;
        }
    }
}