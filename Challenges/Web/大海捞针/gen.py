...
HEAD = open('htmlhead', 'r', encoding='utf-8').read()
TAIL = open('htmltail', 'r', encoding='utf-8').read()
...
# 生成彩蛋号和FLAG号
EASTER_CODES = gen_ea()
FLAG_CODE = random.randint(1, 1000)
while FLAG_CODE in EASTER_CODES:
    FLAG_CODE = random.randint(1, 1000)

data1 = [
    ('notice', '你已加入 MoeCTF2023'),
    ('notice', '昨天 9:23'),
    '师傅带带我！',
...
def import_from_list(content: list):
    res = ''
...
def gen(code: int):
    res = ''
    res += import_from_list(data1)
    if code in EASTER_CODES:
        res += import_from_list(
            [('notice', f'管理员[error, can\'t track user] 撤回了一条消息<!--经过tracker，破获出内容为[色图已屏蔽]-->')])
    elif code == FLAG_CODE:
        res += import_from_list(
            [('notice', f'管理员[error, can\'t track user] 撤回了一条消息<!--经过tracker，破获出内容为{os.getenv("FLAG")}-->')])
    else:
        res += import_from_list(
            [('notice', f'管理员[error, can\'t track user] 撤回了一条消息<!--经过tracker，破获出内容为{random.choice(other_choices)}-->')])
    res += import_from_list(data2)
    return res

for i in range(1,1001):
    BODY = gen(i)
    open(f'templates/ChatLog{str(i)}.html', 'w', encoding='utf-8').write(HEAD + BODY + TAIL)
